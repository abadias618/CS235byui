...
void solveMaze()
{
	/*Graph a(readMaze("C:/Users/abadi/source/repos/Grapg_CS235/maze25x25.txt"));
	vector<Vertex> path;
	int sizeOfGraph = a.size();
	CVertex first = CVertex(0, 1);
	CVertex last = CVertex(sizeOfGraph - 1);

	drawMaze(a, path);
	path = a.findPath(first,last);
	drawMaze(a, path);*/
   // your code goes here.
	vector <Vertex> path;
	std::string fileName;
	...
	cin.get();
	CVertex vStart(0, 1);
	CVertex vEnd(g.size() - 1);

	//findPath dose the real work
	path = g.findPath(vStart, vEnd);
